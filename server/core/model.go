...
func sendmsg(entity EntityInterface, msg *ActionMsg) string {
	if gmodel == nil {
		log.Fatal("model init error")
	}
	...
	resp, err := gmodel.GenerateContent(gctx, prompt)
	if err != nil {
...
func formatActionMsg(you EntityInterface) string {
	...
	baseinfo := fmt.Sprintf("你的ID:%d;名字:%s;类型ID:%d;坐标:[%d,%d],生命值:%d;饱食度:%d;记忆:(%s) ", you.GetId(), you.GetName(), you.GetType(), you.GetX(), you.GetY(), you.GetHP(), you.GetSatietyDegree(), string(record))
	mapInfo := fmt.Sprintf("当前时间:%s,地图信息(二维数组):%s", WorldMap.Gmap.GlobalTime.GetTime(), common.JsonMarshal(WorldMap.Gmap.Map))
	...
	msg = fmt.Sprintf("%s,%s,%s,%s,你可以进行 行为类型 进行交互或者活动,你给出一个你想进行的行为,尽量不要重复,请用JSON格式响应以下请求，不要包含任何解释或格式化字符。JSON格式:{selfid:number,action:ACTION_TYPE,target:{x:number,y:number,id:number},reason:string},例子:{selfid:1001,action:100001,target:{x:0,y:2,id:8888},reason:'你对坐标[0,2]的实体id为8888的树进行了砍伐", baseinfo, mapInfo, actionInfo, entityInfo)
	return msg
}
